...
import { MatrixData, Translations, PersonalGuideResponse, LanguageKey } from '../types';
import { generatePersonalGuide } from '../services/geminiService';
import MatrixDiagram from './MatrixDiagram';
import MarkdownRenderer from './MarkdownRenderer';

interface PersonalResultsScreenProps {
    userName: string;
...
const PersonalResultsScreen: React.FC<PersonalResultsScreenProps> = ({ userName, matrixData, t }) => {
    const [selectedTopic, setSelectedTopic] = useState<keyof PersonalGuideResponse | string>('personality');
    const [aiGuide, setAiGuide] = useState<PersonalGuideResponse | null>(null);
        ...
        const fetchGuide = async () => {
            ...
            try {
                if (process.env.API_KEY) {
                    const content = await generatePersonalGuide(matrixData, userName, 'vi' as LanguageKey);
                    setAiGuide(content);
                } else {
                    // Use the generic error message from translations
                    setApiError(t.apiError);
                }
            } catch (err) {
                // Use the generic error message from translations for any API error
                setApiError(t.apiError);
            } finally {
                setIsLoading(false);
            }
    ...
    }, [matrixData, userName, t.apiError]); // t.apiError added to deps in case it changes, though unlikely for static translations
    ...
    return (
        ...
        >
            ...
            >
                ...
                >
                    <h2 className="text-xl font-semibold text-purple-300 text-center mb-3">{t.diagram}</h2>
                    <MatrixDiagram matrixData={matrixData} />
                </motion.div>
                <motion.div 
                    layout 
                    className="bg-gray-800/50 backdrop-blur-md p-5 rounded-xl shadow-xl border border-white/10"
                    variants={resultsScreenVariants.cardItem}
                >
                    <h2 className="text-xl font-semibold text-purple-300 mb-4">{t.analysisTopics}</h2>
                    <div className="space-y-1.5">
                        {Object.entries(t.topics).map(([key, { title, icon: Icon }]) => (
                            <button
                                key={key}
                ...
                <AnimatePresence mode="wait">
                    {isLoading ? (
                        <motion.div key="loader" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center justify-center h-full text-center flex-grow">
                            ...
                            <p className="mt-4 text-gray-400 text-sm">{t.loadingGuide}</p>
                        </motion.div>
                    ) : apiError ? (
                        ...
                        >
                            <h3 className="text-2xl font-bold text-purple-300 mb-4 sticky top-0 bg-gray-800/80 backdrop-blur-sm py-2 z-10">
                                {t.topics[selectedTopic]?.title || "Phân tích"}
                            </h3>
                            <MarkdownRenderer text={aiGuide?.[selectedTopic as keyof PersonalGuideResponse]} />
                        </motion.div>
                    )}
                </AnimatePresence>
            </motion.div>
...
export default PersonalResultsScreen;